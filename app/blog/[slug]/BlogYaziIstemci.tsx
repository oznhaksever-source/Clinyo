...
export default function BlogYaziIstemci({ yazi, digerYazilar, slug }: any) {
  const { dil } = useDil();
  const d = dil as string;

  function alan(key: string) {
  ...
  const metin: Record<string, Record<string, string>> = {
    ...
    teklifAlt: { tr: "Onaylı kliniklerden kişisel teklif alın.", en: "Get personalized quotes from verified clinics.", de: "Erhalten Sie Angebote von verifizierten Kliniken.", ar: "احصل على عروض من العيادات المعتمدة.", ru: "Получите персональные предложения от клиник.", fr: "Obtenez des devis de cliniques vérifiées." },
  ...
  const t = (key: string) => metin[key]?.[d] || metin[key]?.en || "";
  ...
  return (
    <main style={{ minHeight: "100vh", fontFamily: "'Segoe UI', system-ui, sans-serif", background: "#f8f9ff" }}>
      ...
      <div style={{ maxWidth: "800px", margin: "0 auto", padding: "48px 16px" }}>
        ...
        {/* İlgili yazılar — iç linkleme (SEO için kritik) */}
