...
export default function TeklifDetay({ params }: { params: { id: string } }) {
  ...
  async function teklifOnayla() {
    ...
    await supabase.from("teklifler").update({ durum: "onaylandi" }).eq("id", params.id);
    if (user && klinik?.id) {
  ...
  async function teklifReddet() {
    ...
    await supabase.from("teklifler").update({ durum: "reddedildi" }).eq("id", params.id);
    yukle();
  }
