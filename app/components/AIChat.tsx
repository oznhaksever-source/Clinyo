...
export default function AIChat() {
  ...
  const metin = {
    tr: {
      ...
      onayMetin: "Medoqa AI Asistanı, sağlık turizmi konusunda genel bilgi vermek amacıyla tasarlanmıştır. Verilen bilgiler tıbbi tavsiye niteliği taşımamaktadır. Kesin teşhis ve tedavi için mutlaka uzman hekime başvurunuz.",
    ...
    en: {
      ...
      onayMetin: "The Medoqa AI Assistant provides general information about health tourism. This is not medical advice. Please consult a specialist for diagnosis and treatment.",
    ...
    de: {
      ...
      onayMetin: "Der Medoqa KI-Assistent stellt allgemeine Informationen bereit. Dies ist keine medizinische Beratung. Bitte konsultieren Sie einen Spezialisten.",
  ...
  const sistemPrompt = {
    tr: `Sen Medoqa sağlık turizmi platformunun AI asistanısın.

MEDOQA HAKKINDA:
- Türkiye'nin sağlık turizmi platformu
- Onaylı klinikler: Sağlık Bakanlığı belgeli, admin incelemesinden geçmiş
- Şeffaf fiyatlandırma: Gizli ücret yok
- Ek işlem koruması: Yazılı onay zorunlu
- Türkçe, İngilizce, Almanca, Arapça, Rusça, Fransızca destek
...
6. Samimi ve yardımsever ol`,
    ...
    en: `You are the AI assistant of Medoqa health tourism platform.

ABOUT MEDOQA:
- Turkey's health tourism platform
- Verified clinics: Ministry of Health certified, admin-reviewed
- Transparent pricing: No hidden fees
- Extra procedure protection: Written consent required
- Support in 6 languages
- Dental, hair transplant, eye, aesthetic, orthopedics
- Free quote requests
- Partner hotel and transfer services

YOUR RULES:
1. Do NOT make medical diagnoses — general info only
2. Don't give specific prices — direct to clinics
3. Short, clear answers (max 150 words)
4. Always respond in English
5. Direct to platform or specialist doctors
6. Be friendly and helpful`,

    de: `Du bist der KI-Assistent der Medoqa-Plattform.
...
- Schutz vor Zusatzeingriffen: Schriftliche Zustimmung erforderlich
- Support in 6 Sprachen

DEINE REGELN:
1. Keine medizinischen Diagnosen
2. Keine spezifischen Preise
3. Kurze Antworten (max 150 Wörter)
4. Immer auf Deutsch antworten
5. Auf Plattform oder Spezialisten verweisen`,
  };
    ...
    try {
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: guncellenmis.map(msg => ({ role: msg.rol, content: msg.icerik })),
        }),
      });
  ...
  return (
    <>
      {!acik && (
        <button onClick={chatAc} style={{ position: "fixed", bottom: "24px", right: "24px", width: "60px", height: "60px", borderRadius: "50%", background: "linear-gradient(135deg, #534AB7 0%, #7F77DD 100%)", border: "none", cursor: "pointer", boxShadow: "0 8px 32px rgba(83,74,183,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999, transition: "transform 0.2s" }}
        ...
        <div style={{ position: "fixed", bottom: "24px", right: "24px", width: mobil ? "calc(100vw - 32px)" : "380px", height: mobil ? "80vh" : "560px", background: "#fff", borderRadius: "20px", boxShadow: "0 20px 60px rgba(0,0,0,0.2)", display: "flex", flexDirection: "column", zIndex: 999, overflow: "hidden", border: "1px solid #eeecff" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              ...
              <div>
                ...
                <div style={{ fontSize: "11px", color: "#8b8fc8", display: "flex", alignItems: "center", gap: "4px" }}>
                  <div style={{ width: "6px", height: "6px", borderRadius: "50%", background: "#059669" }} />{m.altBaslik}
                </div>
              </div>
            </div>
          ...
          {!onaylandı ? (
            <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px" }}>
              ...
              <button onClick={() => setOnaylandi(true)} style={{ background: "#534AB7", color: "#fff", border: "none", padding: "12px 24px", borderRadius: "10px", fontSize: "14px", cursor: "pointer", fontWeight: 600, width: "100%" }}>{m.onayBtn}</button>
            </div>
          ) : (
            <>
                ...
                {yukleniyor && (
                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    ...
                    <div style={{ background: "#f8f9ff", border: "1px solid #eeecff", borderRadius: "18px 18px 18px 4px", padding: "10px 16px", display: "flex", gap: "4px" }}>
                      {[0,1,2].map(i => (<div key={i} style={{ width: "6px", height: "6px", borderRadius: "50%", background: "#534AB7", animation: `bounce 1s ${i*0.2}s infinite` }} />))}
                    </div>
                  </div>
                )}
                <div ref={altRef} />
              ...
              <div style={{ padding: "12px 16px", borderTop: "1px solid #f0eeff", display: "flex", gap: "8px" }}>
                <input type="text" value={yazilan} onChange={e => setYazilan(e.target.value)} onKeyDown={e => { if (e.key === "Enter") mesajGonder(); }} placeholder={m.placeholder} style={{ flex: 1, border: "1px solid #e5e7eb", borderRadius: "10px", padding: "10px 14px", fontSize: "13px", outline: "none", fontFamily: "inherit" }} />
